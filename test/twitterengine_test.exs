defmodule TwitterengineTest do
  ...
  test "Registration", %{server: server_pid, clients: clients} do
    ...
    # IO.inspect :ets.lookup(:user_dB, 1)
  ...
  test "Tweet-Sentiment", %{server: server_pid, clients: clients} do
    ...
    contains =Enum.member?(["@parth love @pranav"], elem(Enum.at(:ets.tab2list(:tweet_dB), 0), 2))
    assert contains
    ...
    contains = Enum.member?(["@parth hate @pranav"], elem(Enum.at(:ets.tab2list(:tweet_dB), 0), 2))
    assert !contains
    assert_receive {:trace, ^pid, :receive,
  ...
  test "DoubleHashTag", %{server: server_pid, clients: clients} do
    ...
    assert [{"#COP5615", _}] = :ets.lookup(:hashtag_dB, "#COP5615")
  ...
  test "DoubleMentions", %{server: server_pid, clients: clients} do
    ...
    assert [{"@pranav", _}] = :ets.lookup(:mentions_dB, "@pranav")
    assert [{"@hero", _}] = :ets.lookup(:mentions_dB, "@hero")
  end
  ...
  test "HashTag2", %{server: server_pid, clients: clients} do
    ...
    assert [] = :ets.lookup(:hashtag_dB, "#DOS")
    assert [{"#great", _}] = :ets.lookup(:hashtag_dB, "#great")
  end
  ...
  test "Mentions2", %{server: server_pid, clients: clients} do
    ...
    assert [] = :ets.lookup(:mentions_dB, "@pranav")
    assert [{"@hero", _}] = :ets.lookup(:mentions_dB, "@hero")
  end
  ...
  test "Hashtags and Mentions both", %{server: server_pid, clients: clients} do
    ...
    :sys.get_state(Enum.at(clients, 0))
    :sys.get_state(server_pid)
    assert [] = :ets.tab2list(:mentions_dB)
    ...
    :sys.get_state(Enum.at(clients, 0))
    :sys.get_state(server_pid)
    # IO.inspect ["hehe", :ets.match_object(:tweet_dB, {:"_", 1, :"_"})]
    # IO.inspect [ "haha", GenServer.call(Enum.at(clients,0),{:queryMyMention,clientName})]
    assert [{"@subham", _}] = :ets.lookup(:mentions_dB, "@subham")
  ...
  test "Subscribe", %{server: server_pid, clients: clients} do
    ...
    assert [{1, [2], [], "connected", 0}] = :ets.lookup(:user_dB, 1)
  ...
  test "Retweet and Subscribed user receiving message", %{server: server_pid, clients: clients} do
    ...
    {_, _, client1_retweet} = Enum.at(:ets.match_object(:tweet_dB, {:_, 1, :_}), 0)
    {_, _, client2_tweet} = Enum.at(:ets.match_object(:tweet_dB, {:_, 2, :_}), 0)
  ...
  test "Live-tweets test2", %{server: server_pid, clients: clients} do
    ...
    assert [{1, ["2T2"]}] = :ets.lookup(:msgQueue_dB, 1)
