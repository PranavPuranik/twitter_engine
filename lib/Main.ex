defmodule Main do
    ...
    receive do
      ...
      {:registered} ->
        ...
        if count == numClients do
          ...
          Enum.map(nodeid_list, fn x ->
            GenServer.cast(String.to_atom(x), {:simulate, tweet_pool})
          end)
        end
    end
  ...
  def subscribe(nodeid_list, clients) do
    ...
    Enum.map(nodeid_list, fn id ->
      ...
      subscribers =
        Enum.take_random(
          all_clients -- [elem(Integer.parse(String.slice(id, 7..-1)), 0)],
      ...
      Enum.each(subscribers, fn n ->
        GenServer.cast(
          String.to_atom("client_" <> Integer.to_string(n)),
